...
%present the final 
Now that the most feasible option for each vehicle passenger range has been selected, they . For clarity an overview of the inputs and outputs for the winning concepts in each category are shown below in \autoref{inputswin}. 
...
For the highest risks there are things that can be done to mitigate these risks. Examples are are as follows. For 1, make a conservative first guess of the MTOW. For 4, Considered a smaller and wealthier target group. For 6, promote UAM safety awareness through advertisements and demonstrations. For 7, ensure the fire is contained, plan emergency procedures and test protective casings. For 8 and 9, keep open the option to use swappable batteries until the detailed design phase. For 10, adjust the route networks in such a way that airspace needed does not interfere with flightpaths.   
